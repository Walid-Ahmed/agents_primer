"""Small logging helpers shared by the OpenAI examples."""
...
def log_message(log_file: TextIO, role: str, content: object) -> None:
    ...
    # SDK response items may be Python objects. Convert them to ordinary
...
def execution_log(script_file: str) -> Iterator[TextIO]:
    ...
    # openai folder, so logs are stored in openai/logs/.
    ...
    try:
        ...
        with log_path.open("w", encoding="utf-8") as log_file:
            log_file.write(f"OpenAI tool-use execution: {datetime.now()}\n")
            yield log_file
    finally:
